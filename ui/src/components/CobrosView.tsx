...
export default function CobrosView(){
  ...
  async function pay(tid:string, n:number, raw:string){ const a=parseLocaleNumber(raw,false); if(a<=0) return; const r=await repo.receivablePay(tid,n,a); alert(r.ok?'Cobro OK':'Error'); }
  return (<section>
    <h2>Cobros</h2>
    <h3>Pendientes</h3>
    <table style={{width:'100%'}}><thead><tr><th>Vence</th><th>Ticket</th><th>Cliente</th><th>#</th><th style={{textAlign:'right'}}>Monto</th><th style={{textAlign:'right'}}>Entrega parcial</th></tr></thead>
      <tbody>{pend.map((r,i)=>(<tr key={i}>
        <td>{new Date(r.fecha).toLocaleDateString()}</td><td>{r.ticketId}</td><td>{r.cliente}</td><td>{r.cuota}</td>
        <td style={{textAlign:'right'}}>{fmtGs.format(r.monto||0)}</td>
        <td style={{textAlign:'right'}}>
          <input type="text" inputMode="numeric" placeholder="0"
            onKeyDown={e=>{ if(e.key==='Enter'){ const v=(e.target as HTMLInputElement).value; pay(r.ticketId, r.cuota, v); (e.target as HTMLInputElement).value=''; } }}
            style={{width:120}}/>
        </td>
      </tr>))}</tbody>
    </table>
